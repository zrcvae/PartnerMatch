...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public List<User> searchUsersByTags(List<String> tagNameList) {
        ...
        return userList.stream().filter(user -> {
            ...
            Set<String> userTags = gson.fromJson(tagStr, new TypeToken<Set<String>>() {
            }.getType());
            // java8新特性
    ...
    public int updateUser(User user, User loginUser) {
        ...
        // 根据id查询数据库信息，再对比传入的user是否有变化
        User oldUser = userMapper.selectById(userId);
        if (oldUser != null && (oldUser.getEmail().equals(user.getEmail()) &&
                oldUser.getPhone().equals(user.getPhone()) &&
                oldUser.getAvatarUrl().equals(user.getAvatarUrl()) &&
                oldUser.getGender().equals(user.getGender()) &&
                oldUser.getTags().equals(user.getTags()) &&
                oldUser.getUserRole().equals(user.getUserRole()) &&
                oldUser.getUserStatus().equals(user.getUserStatus()) &&
                oldUser.getPlanetCode().equals(user.getPlanetCode()) &&
                oldUser.getUsername().equals(user.getUsername()))) {
            throw new BusinessException(ErrorCode.PARAMS_EXISTS_ERROR);
        }
        // 如果是管理员，允许更新任意用户
    ...
    public List<User> matchUsers(long num, User loginUser) {
        ...
        for (int i = 0; i < userList.size(); i++) {
            ...
            long distance = AlgorithmUtils.minDistance(tagList, userTags);
        ...
        Map<Long, List<User>> unOrderUserList = this.list(queryWrapper).stream().
                map(user -> this.getSafetyUser(user)).
                collect(Collectors.groupingBy(user -> user.getId()));
        List<User> finalUserList = new ArrayList<>();
        for (Long userId : userIdList) {
            finalUserList.add(unOrderUserList.get(userId).get(0));
        }
